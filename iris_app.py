...
rfclf=RandomForestClassifier(n_jobs=-1,n_estimators=100)
...
if st.sidebar.button("Predict"):
  if classifier == 'Support Vector Machine':
    ...
    species_type = prediction(lr, s_len, s_wid, p_len, p_wid)
    score = lr.score(X_train, y_train)

  else:
    species_type = prediction(rfclf, s_len, s_wid, p_len, p_wid)
    score = rfclf.score(X_train, y_train)
